    ...
    int
    BitMap::FindBestFit(int required){
        int gap = NumPhysPages;
        int gapIndex = 0;
        bool inGap = FALSE;
        int tempInd = 0;
        int tempGap = 0;
        ...
        if (!Test(i)){
            ...
            inGap = FALSE;
            if(tempGap < gap && tempGap >= required){
            gap = tempGap;
            gapIndex = tempInd;
            }
            tempGap = 0;     
    ...
    int
    BitMap::FindWorstFit(int required){
        int gap = 0;
        int gapIndex = 0;
        bool inGap = FALSE;
        int tempInd = 0;
    ...
    int
    BitMap::FindFirstFit(int required){
        ...
        if (!Test(i)) {
            tempS = i;
            while((!Test(i))&& found <= required && i<NumPhysPages){
            found++;
            i++;
    ...
    //      return i;
            ...
            if(found<required){
            found =0;               //reset found if not enough space
            }else{
            for(int j = 0; j< required; j++){
                Mark(tempS + j);
