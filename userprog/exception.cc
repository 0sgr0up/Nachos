...
ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException :
		...
		case SC_Exec :
		{
			printf("This is kinda working??!!");

			int buffadd = machine->ReadRegister(4);

			printf("This is the buffadd %d\n", buffadd);
			char *filename = new char[100];

			if(!machine->ReadMem((int) buffadd,1, (int  *) ch)) return;
			i =0;
			...
			printf("The executable being read is: %s", filename);
			...
			// Thread * t = new Thread("The Thread");
			// t->space = space;
			...
			// int processedId = 2;
			// machine->WriteRegister(2, processedId);
